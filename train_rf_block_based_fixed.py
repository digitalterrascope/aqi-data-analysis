...
MODEL_PATH = "rf_model_block_based.pkl"
...
# Analyze blocks (using pl.len() instead of deprecated pl.count())
...
for block_id in usable_blocks["block_id"].to_list():
    ...
    block_df = block_df.with_columns([
        ...
        pl.col("PM2_5_UGM3").shift(12).alias("lag_12"),  # ~12 readings back
        pl.col("PM2_5_UGM3").shift(24).alias("lag_24"),  # ~24 readings back
    ])

    # Create rolling statistics WITHIN this block
    # Convert to pandas for easier rolling calculations
    block_pd = block_df.to_pandas().sort_values('last_updated')
    ...
    for window_minutes in [15, 30, 60, 120, 360, 720, 1440]:
        ...
        for idx, row in block_pd.iterrows():
            ...
            mask = (block_pd['last_updated'] >= window_start) & \
                   (block_pd['last_updated'] < current_time)
...
lag_features = ["lag_1", "lag_2", "lag_3", "lag_4", "lag_5", "lag_6", "lag_12", "lag_24"]
...
all_features = []
for feature_list in [lag_features, rolling_features, time_features, sensor_features]:
    all_features.extend([f for f in feature_list if f in combined_df.columns])

print(f"Total features: {len(all_features)}")
print(f"Features: {all_features[:10]}...")  # Show first 10
